...
from panel_api.schemas import AuditLogOut, BackfillCitationsResponse, CommitResponse, ComparativeImportResponse, ReextractRequest, ReextractResponse, VocabBaseSyncResponse
...
from panel_api.services.comparative_import import import_comparative_all
...
def _rules_capture_audit_path() -> Path:
    explicit = (os.environ.get("WOCCON_RULES_CAPTURE_AUDIT_PATH") or "").strip()
    if explicit:
        return Path(explicit)
    return Path(__file__).resolve().parents[2] / "data" / "backups" / "rules_capture_audit.json"


@router.get("/rules-capture-audit")
def get_rules_capture_audit(admin: RequireAdmin):
    """Serve latest rules_capture_audit.json generated by scripts/rules_capture_audit.py."""
    path = _rules_capture_audit_path()
    if not path.is_file():
        raise HTTPException(
            status_code=404,
            detail="Rules capture audit not found. Run scripts/rules_capture_audit.py locally.",
        )
    try:
        return json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as e:
        raise HTTPException(status_code=500, detail=f"Invalid audit JSON: {e}") from e


@router.post("/import-comparative", response_model=ComparativeImportResponse)
def import_comparative(db: DbSession, admin: RequireAdmin):
    result = import_comparative_all(db)
    db.add(
        AuditLog(
            entity_type="comparative_import",
            entity_id="batch",
            action="import",
            user_id=admin.id,
    ...
    db.commit()
    return ComparativeImportResponse(**result)
